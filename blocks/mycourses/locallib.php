...
function mycourses_get_my_completion($datefrom = 0) {
    ...
    $mycompleted = $DB->get_records_sql("SELECT cc.id, cc.userid, cc.courseid as courseid, gg.finalgrade as finalgrade, c.fullname as coursefullname, c.summary as coursesummary
                                         FROM {local_iomad_track} cc
                                         JOIN {course} c ON (c.id = cc.courseid)
										 inner join {grade_items} gi on gi.courseid = c.id inner join {grade_grades} gg on  gi.id = gg.itemid and gg.userid= cc.userid
                                         WHERE cc.userid = :userid
                                         AND c.visible = 1
                                         AND cc.timecompleted > :datefrom group by c.id",
	 ...
	 $myinprogress = $DB->get_records_sql("SELECT cc.id, cc.userid, cc.course as courseid, c.fullname as coursefullname, c.summary as coursesummary
                                          ...
                                          AND ue.timestart != 0  group by c.id",
    ...
    foreach ($mycompleted as $id => $completed) {
		...
		$arrcourseid[]= $completed->courseid;
        // Deal with the iomadcertificate info.
        if ($hasiomadcertificate) {
            if ($iomadcertificateinfo = $DB->get_record('certificate',
                ...
                $certstring = "<a href='".$CFG->wwwroot."/mod/certificate/view.php?id=".
                              ...
                              sesskey()."' target='_blank'>".get_string('downloadcert', 'block_mycourses').
                              "</a>";
            } else {
	 ...
	 $mycoursenotstartedsql = "SELECT  c.id as courseid,ue.userid, c.fullname as coursefullname, c.summary as coursesummary
		  FROM
            {user} u
        INNER JOIN
            {user_enrolments} ue
        ...
        AND crc.timeenrolled IS NULL
		  and u.id = :userid ";
		  if(count($arrcourseid) > 0)
		  $mycoursenotstartedsql .= " and c.id not in (".implode(",",$arrcourseid).")";
		  
		$mycoursenotstartedsql .="   group by c.id";
		  
	$mycoursenotstarted =$DB->get_records_sql($mycoursenotstartedsql,array('userid' => $USER->id));

    $mycompletions->mycompleted = $mycompleted;
    $mycompletions->myinprogress = $myinprogress;
    $mycompletions->mynotstartedenrolled = $mycoursenotstarted;
...
function mycourses_get_my_archive($dateto = 0) {
    ...
    $mycompletions = new stdclass();
    $myarchive = $DB->get_records_sql("SELECT cc.id, cc.userid, cc.courseid as courseid, gg.finalgrade as finalgrade, c.fullname as coursefullname, c.summary as coursesummary
                                       FROM {local_iomad_track} cc
                                       JOIN {course} c ON (c.id = cc.courseid)
									    inner join {grade_items} gi on gi.courseid = c.id inner join {grade_grades} gg on  gi.id = gg.itemid and gg.userid= cc.userid
                                       WHERE cc.userid = :userid
