...
$sql = "SELECT ci.timecreated AS citimecreated,
    ...
    INNER JOIN {certificate} c
		...
		inner join {grade_items} gi on  c.course = gi.courseid 
		inner join {grade_grades} gg on  gi.id = gg.itemid and gg.userid= u.id
		inner join  {user_enrolments} mue on mue.userid = u.id 
		inner join  {enrol} me on me.id = mue.enrolid 
		where gi.itemtype ='mod' and gg.rawgrade >= 80 and ci.code = ?
