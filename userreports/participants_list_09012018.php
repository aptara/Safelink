				...
				foreach($courses as $course)
				{
					...
					foreach ($modinfo->sections as $sectionnum=>$section)
					{
						...
						foreach ($section as $cmid)
						{
							...
							if($modulename =='SCORM package' || $modulename =='Paquete SCORM')
							{
								$sql = "SELECT s.id,GROUP_CONCAT(value ORDER BY attempt DESC SEPARATOR '%, ') as score  from {scorm} s INNER JOIN {scorm_scoes_track} sst ON s.id = sst.scormid  WHERE  s.id = ? and  sst.element = ?  AND sst.userid = ? ORDER BY sst.timemodified DESC";
