...
$sql="select mu.* , comp.name as companyname,me.courseid,dept.name as department,certissue.code,certissue.timecreated as certissuedate,cert.id as certid, certissue.id as certissueid,course.fullname from {user} mu 
		...
		inner join {grade_items} gi on gi.courseid = course.id and cert.course = gi.courseid 
		inner join {grade_grades} gg on  gi.id = gg.itemid and gg.userid= mu.id
		where gi.itemtype ='mod' and gg.rawgrade >= 80 and
		 mue.status = 0 and  ra.roleid = ? and mu.deleted = ? and  me.courseid  in (select iocourse.courseid from {iomad_courses} iocourse left join {company_course} cocourse on cocourse.courseid = iocourse.courseid where  shared = 1 or (cocourse.companyid = ".(int) $USER->company->id." and shared = 0))
		...
		if(user_has_role_assignment($USER->id, DEPARTADMIN))
		{
		    ...
		    $sql.="	  order by certissuedate desc,".$sort." ".$dir;
