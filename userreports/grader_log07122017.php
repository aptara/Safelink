...
$content =$OUTPUT->heading('Grader Log');
		...
		$sql = "SELECT gi.*
                 ...
                 WHERE   gi.courseid in  (select iocourse.courseid from {iomad_courses} iocourse left join {company_course} cocourse on cocourse.courseid = iocourse.courseid where shared = 1 or (cocourse.companyid = ".(int) $USER->company->id." and shared = 0)) and (itemname is not NULL) and itemname !='' order by sortorder,itemname ";
		$grade_items =$DB->get_records_sql($sql,array(2));
		
		$table->head  = array( get_string('name'),get_string('email'),get_string('company'));
		foreach($grade_items as $grade_item)
		{
			$table->head[] = $grade_item->itemname;
		}
		$data = array();
		$sql_grade = "SELECT g.*
		  FROM {grade_items} gi INNER JOIN
