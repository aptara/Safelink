...
$sql="select mu.* , comp.name as companyname,me.courseid,dept.name as department,certissue.code,certissue.timecreated as certissuedate,cert.id as certid, certissue.id as certissueid from {user} mu 
...
$columns = array(get_string("fullname"),get_string("email"),get_string("company"),get_string("department"),get_string("lastaccess"),get_string("downloadcertificatescode"),get_string("downloadcertificatesdate"));
   ...
   // $table->head  = array_merge($columns,Array('Action'));	
    ...
    if(count($users) > 0)
    {
            foreach ($users as $user)
            {
				...
				$row1 =array();
				//print_object($user);
				$row1 = array(fullname($user),$user->email,$user->companyname,$user->department,$datestring,$user->code,userdate($user->certissuedate).certificate_print_user_files($certificate, $user->id, $certcontext->id));
